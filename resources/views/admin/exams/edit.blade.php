    ...
    <div class="col-xs-12">
        <div class="box box-success">
            <div class="box-header with-border">
                <h3 class="box-title">تعديل موعد اختبار</h3>
            </div>
                    ...
                    <div class="form-group">
                        ...
                        <select  id="grade" name="grade_id" class="form-control">
                            @foreach($grades as $k=> $grade)
                                <option value="{{$grade->id }}" {{$grade->id == old('grade_id',$exam->grade_id) ? ' selected="selected"' : '' }} >{{$grade->name}}</option>                                
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select  id="room" name="room_id" class="form-control">
                            @foreach($rooms as $k=> $room)
                                <option value="{{$room->id }}" {{$room->id == old('room_id',$exam->room_id) ? ' selected="selected"' : '' }} >{{$room->name}}</option>
                    ...
                    <div class="form-group">
                        ...
                        <select  id="subject" name="subject_id" class="form-control">
                            @foreach($subjects as $k=> $subject)
                                <option value="{{$subject->id }}" {{$subject->id == old('subject_id', $exam->subject_id) ? ' selected="selected"' : '' }} >{{$subject->name}}</option>
                            @endforeach
                        </select>
                    </div>                   
                </div>
